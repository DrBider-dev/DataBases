...
# =========================================================
# "auto": True cuando la columna se genera automáticamente en la base de datos.
# "fk": nombre de la tabla referenciada para llenar combobox.
...
class CRUDApp(tk.Tk):
    ...
    def rebuild_form(self) -> None:
        ...
        for col in meta["columns"]:
            ...
            if is_pk and auto:
                # Dejar visible pero no editable al crear.
                try:
    ...
    def load_data(self) -> None:
        ...
        try:
            rows = self.db.fetchall(query.as_string(self.db.conn))
        except Exception as exc:
            self.db.rollback()
    ...
    def parse_value(self, value: str, col_meta: Dict[str, Any]) -> Any:
        ...
        if col_type == "numeric":
            return float(value)
        if col_type == "date":
            return value  # formato YYYY-MM-DD
    ...
    def create_record(self) -> None:
        ...
        try:
            ...
            if not cols:
                messagebox.showwarning("Aviso", "No hay datos para insertar.")
                return
    ...
    def update_record(self) -> None:
        ...
        try:
            ...
            update_cols = [c["name"] for c in meta["columns"] if c["name"] != pk and not c.get("auto", False)]
            sets = []
            params = []
            ...
            for col_name in update_cols:
                if col_name not in data:
                    continue
                sets.append(sql.SQL("{} = {}").format(sql.Identifier(col_name), sql.Placeholder()))
                params.append(data[col_name])

            if not sets:
                messagebox.showwarning("Aviso", "No hay campos para actualizar.")
                return
    ...
    def delete_record(self) -> None:
        ...
        try:
            ...
            self.db.execute(query.as_string(self.db.conn), (pk_value,))
    ...
    def search_by_pk(self) -> None:
        ...
        try:
            ...
            row = self.db.fetchone(query.as_string(self.db.conn), (pk_value,))
            if row is None:
